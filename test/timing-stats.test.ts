...
describe('createTimingStats', () => {
  ...
  it('rejects invalid configuration and handles no completed tests', () => {
    expect(() => createTimingStats([], { binSizeMs: 0 })).toThrow('binSizeMs must be greater than 0')
    expect(() => createTimingStats([
      { name: 'long', file: 'fixture.test.ts', durationMs: 300_000 },
    ], { binSizeMs: 1 })).toThrow('binSizeMs would produce 300001 histogram bins')
