...
export function createTimingStats(
  ...
  const completedTests = tests.filter(test => Number.isFinite(test.durationMs) && test.durationMs >= 0)
  ...
  if (binCount > maxHistogramBins) {
    throw new Error(`binSizeMs would produce ${binCount} histogram bins; increase it to produce at most ${maxHistogramBins}`)
  }
  ...
  return {
    ...
    slow: {
      ...
      percentage: completedTests.length === 0 ? 0 : (slowTests.length / completedTests.length) * 100,
...
export function formatTimingStats(stats: TimingStats): string {
  ...
  const widestBinLabel = Math.max(...stats.histogram.map(bin => `${bin.startMs}-${bin.endMs}ms`.length))
  const lines = [
    '',
  ...
  for (const bin of stats.histogram) {
    ...
    lines.push(`  ${label}  ${bar.padEnd(24)}  ${formatPercent(bin.percentage).padStart(5)}  ${bin.count}`)
  }
  ...
  if (stats.slowest.length > 0) {
    ...
    for (const [index, test] of stats.slowest.entries()) {
      lines.push(`  ${index + 1}. ${formatMilliseconds(test.durationMs).padStart(7)}  ${test.file} > ${test.name}`)
    }
  }
